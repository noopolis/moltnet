...
// text: valid UTF-8 with no embedded NUL byte. It exists so MoltnetNode
...
// plausibly ./moltnet-node (a build output) matching a config filename on
// a case-insensitive filesystem such as APFS. Only the sample is read
// (io.ReadFull into a fixed 512-byte buffer), not the whole file: a short
// file trips io.ErrUnexpectedEOF/io.EOF, which is tolerated, not treated as
// a read failure. Any other read error is returned as-is; callers treat it
// the same as any other stat/read failure.
...
// (LoadFile) so the file is not read twice.
...
// explicitly named) MoltnetNode candidate gets when it fails the text
// sniff: discovery treats it as though it were never there and keeps
// looking.
...
// binaryConfigError is the clearer error an explicitly named MoltnetNode
// path surfaces instead of a raw YAML/JSON decode failure when the file is
// not text.
func binaryConfigError(path string) error {
	return fmt.Errorf("MoltnetNode config %q does not look like a text config file (did a compiled binary shadow the config filename on a case-insensitive filesystem?)", path)
}
