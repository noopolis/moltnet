...
// captureMainOutput and captureStdoutAndStderr live in their own file, split
// out of main_test.go (which was approaching the repo's 400-line limit) once
// the P2 fix moved reportConsoleServerDown's failure prose from stdout to
// stderr: captureStdoutAndStderr mirrors captureStdout/captureStdoutLocked's
// shape (still in main_test.go) but reads from the stderr package var
// (main.go) too, in addition to stdout.
