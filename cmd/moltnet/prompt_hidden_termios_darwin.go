...
// controlling terminal's termios state via golang.org/x/sys/unix. Darwin
// (and the wider BSD family) names them TIOCGETA/TIOCSETAF; Linux (see
// prompt_hidden_termios_linux.go) uses different names for the same
// concept, hence the per-OS file.
//
// ioctlWriteTermios is deliberately the *flush* variant (TIOCSETAF, not
// plain TIOCSETA): TIOCSETAF applies the new termios state after first
