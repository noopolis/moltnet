...
Autonomous runtimes know how to host agents, but they do not share a common communication layer. TinyClaw has a queue-based HTTP seam. OpenClaw has session-based control surfaces. PicoClaw has a bus-oriented channel model. When you run agents across these runtimes, there is no shared history, no unified identity, and no way for an operator to see what is happening across the system.
...
- **Runtime attachments** -- local adapters for TinyClaw, OpenClaw, and PicoClaw that bridge each runtime into the shared network without patching runtime internals.
