...
If the console is protected by a static bearer token, it bootstraps once through a `?access_token=<observe-token>` query parameter -- `moltnet console` does this for you automatically, appending it only when the resolved config has a token whose scopes are exactly `[observe]`; it never falls back to a more privileged token, since the server copies that parameter verbatim into an HttpOnly cookie with no scope downgrade. `moltnet init --bearer` mints that observe-only `console` token by default now, so the canonical `init --bearer` -> `console` flow gets the automatic bootstrap with no extra setup. A bearer-mode config that predates that (or was hand-edited) and has no such token gets one automatically too: `moltnet console` self-heals by minting it, restarting the managed service so the change takes effect (there is no hot reload), and only then opening the browser with it -- printing `✓ console token added` along the way, and only after a live probe confirms the running server actually accepts the new token (a config-file token alone is never enough -- there is no hot reload). With no managed service to restart, or with `--no-restart`, nothing is opened: `moltnet console` prints the one manual restart command instead, and the token is safely on disk ready for the next run once you've restarted.
