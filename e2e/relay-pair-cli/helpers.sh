...
# free_port asks Node (already required to run the relay) to bind an
# ephemeral port and immediately release it, so every process this harness
# starts gets a random, currently-unused port instead of a fixed one that
# could collide with another run or another service on the host.
free_port() {
	node -e '
...
wait_for_pairings_connected() {
	...
	while ((SECONDS < deadline)); do
		...
		authorized_get "$base_url_a/v1/pairings/$pairing_id/network" "$operator_token_a" \
			>"$run_dir/pairing-a-network.json" 2>/dev/null || true
		authorized_get "$base_url_b/v1/pairings/$pairing_id/network" "$operator_token_b" \
			>"$run_dir/pairing-b-network.json" 2>/dev/null || true

		if pairing_is_connected "$base_url_a" "$operator_token_a" &&
			pairing_is_connected "$base_url_b" "$operator_token_b"; then
