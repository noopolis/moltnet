...
This resolves the network's config, health-checks the server first, and opens the console for you -- appending an `?access_token=...` query parameter only when the config has a token whose scopes are exactly `[observe]`, never a more privileged one. `moltnet init --bearer` mints that observe-only `console` token by default now, so this works with no further setup on the canonical `init --bearer` -> `console` flow; a bearer-mode config with no such token gets one self-healed in automatically (a fresh token minted, written, and the managed service restarted so it takes effect) rather than opening straight onto a 401. Either way, the token is always probed against the live server before it is ever put in the browser URL; when that probe fails (a restart never happened, or never came back), nothing is opened, and the exact command to fix it is printed instead. See [CLI reference](/reference/cli/#moltnet-console) for `--print`/`--no-open`/`--no-restart`.
